...
void read_inputs(){
  ...
  //if (debug) Serial.println(mozziAnalogRead(MODE_CV_PIN));
...
void ADD_setFreqs(){
 ...
 ADD_freqv1 = ADD_freq1 * volts_to_frequency((float) v_oct_val); //pow(2, (pgm_read_float(&(voctpow[v_oct_val])))); // V/oct apply
...
void check_modes(){
  ...
  if (mode_val==0) { // toggle mode state 
    last_mode_val = last_mode_val + 1;
    if (last_mode_val > 200) {
      if (debug) Serial.println("Ahah"); 
...
void updateControl() {
 ...
 if (p1_pot_val >= 1020) { //inv knob max
  ...
  if(mode==0){
    knob_freq = freq_val;
    FM_setFreqs((2270658 + knob_freq * 5000) * volts_to_frequency((float) v_oct_val)); //pow(2, (pgm_read_float(&(voctpow[v_oct_val])))));
;
  }
  else if(mode==2){
    ADD_setFreqs();
